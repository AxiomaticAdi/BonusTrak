...
- **Routes** (`app/`): `/` is the dashboard (`app/page.tsx`), `/settings` holds the editors. `app/layout.tsx` wraps everything in `SupabaseProvider`, `AuthGate`, `StoreProvider`, `LinkedGate`, `OfflineIndicator`, and the Sonner `Toaster`.
